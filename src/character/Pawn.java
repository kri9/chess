...
public class Pawn extends Piece{
	...
	public void getValidMoves(boolean[][] moveArray, int y, int x) {
		System.out.println("Получение возможных ходов для Пешки");

		if(this.color == Color.BLACK){
			if(y + 1 <= 7) moveArray[x][y+1] = isEmpty(y + 1, x);
			...
			if(hasNotMoved) moveArray[x][y+2] = isEmpty(y + 2, x) && isEmpty(y + 1, x);
		} else {
			if(y - 1 >= 0) moveArray[x][y-1] = isEmpty(y - 1, x) && isEmpty(y - 1, x);
			...
			if(hasNotMoved) moveArray[x][y-2] = isEmpty(y - 1, x) && isEmpty(y - 1, x);
		}		
	}
}
