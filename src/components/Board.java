...
public class Board {
	...
	public Board() {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++){
				...
				if (i % 2 == 0) {
					if (j % 2 == 0) {
						boardArray[i][j] = tile = new Square(new Empty(Color.GREEN), Color.WHITE);	
					} else {
						boardArray[i][j] = tile = new Square(new Empty(Color.GREEN), Color.BLACK);		
					}
				} else {
					if (j % 2 == 0) {
						boardArray[i][j] = tile = new Square(new Empty(Color.GREEN), Color.BLACK);
	...
	public void setSquareCardinality(){
